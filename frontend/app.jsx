    ...
    function ProgressBar({ atual, total }) {
      ...
      return (
        <div className="w-full bg-gray-200 rounded-full h-2">
          <div className="bg-gray-900 h-2 rounded-full transition-all duration-500"
               style={{ width: `${pct}%` }} />
        </div>
      );
    }
    ...
    function FatorLabel({ codigo, nome }) {
      return (
        <div className="flex items-center gap-2 mb-4">
          <span className="text-xs font-mono font-bold text-white bg-gray-900
                           px-2 py-0.5 rounded-md">{codigo}</span>
              ...
              <tr>
                ...
                {[0,1,2,3,4].map(n => (
                  <th key={n} className="text-center text-xs font-semibold text-gray-500 pb-2 px-2">
                    <div>{n}</div>
                    {n === 0 && <div className="font-normal text-gray-400">Muito ruim</div>}
                    {n === 4 && <div className="font-normal text-gray-400">Muito bom</div>}
                  </th>
                ))}
              </tr>
            </thead>
      ...
      return (
        <Card>
          ...
          {carregando ? (
            ...
            <div className="space-y-4 overflow-y-auto pr-1" style={{ maxHeight: "40vh" }}>
              {Object.entries(visitas).map(([visita, arquivos]) => (
                <div key={visita}>
                  ...
                  <ul className="space-y-1.5">
                    {arquivos.map(pdf => (
                      <li key={pdf.caminho}>
                        <a href={urlPdf(pdf.caminho)}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="flex items-start gap-1.5 text-xs text-blue-600
                                     hover:text-blue-800 hover:underline"
                          title={pdf.nome}>
                          <span className="shrink-0 mt-0.5">📄</span>
                          <span>{labelPdf(pdf.nome)}</span>
                        </a>
                      </li>
                    ))}
                  </ul>
    ...
    function ResumoPainel({ texto, onFechar }) {
      return (
        <div className="bg-white rounded-2xl shadow p-6 flex flex-col overflow-auto"
             style={{ height: "40vh", minHeight: "8rem", maxHeight: "85vh", resize: "both" }}>
          <div className="flex items-center justify-between mb-4 shrink-0">
            <h3 className="text-sm font-semibold text-gray-800">Resumo de Alta</h3>
            <button onClick={onFechar}
              className="text-gray-400 hover:text-gray-700 text-sm leading-none transition"
              title="Fechar painel">
              ✕
    ...
    function App() {
      ...
      const painelAberto = sidebarAberta || resumoAberto;
      const wrapperClass = painelAberto
        ? "mx-auto py-8 px-4 max-w-6xl"
        : "mx-auto py-8 px-4 max-w-3xl";
        ...
        <div className="min-h-screen bg-gray-100">
          ...
          <div className={wrapperClass}>
            <div className={painelAberto ? "flex gap-5" : ""}>
              ...
              {/* Coluna principal */}
              <div className={`${painelAberto ? "flex-1 min-w-0" : ""} space-y-5`}>
                ...
                {/* Resumo inline (oculto quando painel lateral está aberto) */}
                {!resumoAberto && (
                  <div className="bg-white rounded-2xl shadow p-6 flex flex-col overflow-auto"
                       style={{ height: "20rem", minHeight: "8rem", maxHeight: "85vh", resize: "both" }}>
                    <h2 className="font-semibold text-gray-700 mb-3 shrink-0">Resumo de Alta</h2>
                    <div className="whitespace-pre-wrap text-sm text-gray-800 leading-relaxed
                                    overflow-y-auto flex-1 bg-gray-50 rounded-xl p-4
              ...
              {/* Coluna direita: Resumo e/ou Documentos */}
              {painelAberto && (
                <div className="w-80 shrink-0">
                  <div className="sticky top-4 space-y-4">
                    {resumoAberto && (
                      <ResumoPainel
                        texto={resumoAtual.texto}
